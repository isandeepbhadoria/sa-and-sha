...
export async function getInvoiceNumberingConfigFromFirestore(db?: any): Promise<InvoiceNumberingConfig> {
  if (db) {
    try {
      const docRef = db.collection("invoice_number_config").doc("default");
      ...
      console.warn("Failed to load invoice numbering config from Firestore:", err);
    }
  }
  return getDefaultInvoiceNumberingConfig();
}
