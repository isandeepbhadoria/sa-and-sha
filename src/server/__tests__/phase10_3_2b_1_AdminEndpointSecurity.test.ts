...
describe("Phase 10.3.2B.1 Admin Invoice Security & Lifecycle Regression Tests", () => {
  ...
  it("2. Reject non-admin / invalid email authorization", async () => {
    ...
    const isAuthorized = invalidEmail === "shop@saandsha.com";
    expect(isAuthorized).toBe(false);
  });

  it("3. Accept valid shop@saandsha.com admin session", async () => {
    const validEmail = "shop@saandsha.com";
    const isAuthorized = validEmail === "shop@saandsha.com";
  ...
  it("5. Authenticated admin + pre-dispatch COD order rejected by eligibility engine", async () => {
    ...
    const result = await finalizeGstInvoiceForOrder(db, "KL-COD-PRE", { createdBy: "admin:shop@saandsha.com" });
  ...
  it("6. Authenticated admin + cancelled order rejected by eligibility engine", async () => {
    ...
    const result = await finalizeGstInvoiceForOrder(db, "KL-CANCELLED-1", { createdBy: "admin:shop@saandsha.com" });
  ...
  it("7. Authenticated admin + eligible prepaid order (processing/fulfillment) succeeds", async () => {
    ...
    const result = await finalizeGstInvoiceForOrder(db, "KL-ELIG-PREPAID", { createdBy: "admin:shop@saandsha.com" });
  ...
  it("8. Authenticated admin + dispatched COD order succeeds", async () => {
    ...
    const result = await finalizeGstInvoiceForOrder(db, "KL-ELIG-COD", { createdBy: "admin:shop@saandsha.com" });
  ...
  it("9. Duplicate admin request returns same invoice with reused=true", async () => {
    ...
    const result = await finalizeGstInvoiceForOrder(db, "KL-DUP", { createdBy: "admin:shop@saandsha.com" });
  ...
  it("10. Duplicate request allocates zero additional invoice numbers", async () => {
    ...
    const result1 = await finalizeGstInvoiceForOrder(db, "KL-DUP2", { createdBy: "admin:shop@saandsha.com" });
    const result2 = await finalizeGstInvoiceForOrder(db, "KL-DUP2", { createdBy: "admin:shop@saandsha.com" });
